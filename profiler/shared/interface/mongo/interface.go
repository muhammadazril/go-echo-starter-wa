...
func NewClient(conf *viper.Viper) (Client, error) {

	user := conf.GetString(models.ConfigMongoUsername)
	password := conf.GetString(models.ConfigMongoPassword)
	...
	time.Local = time.UTC
	c, err := mongo.NewClient(options.Client().ApplyURI(mongodbURI).SetRegistry(rb.Build()).SetMonitor(apmmongo.CommandMonitor()))
